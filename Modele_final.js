...
function simulation(telecharg){
    ...
    */
    ...
    function modele(Nb_max_rames, Tps_sim, Distance_securite, w, Distances_interstations, Vitesse_max, Vitesse_moy, Num_modele){
        ...
        var min_li= Math.min(Nb_max_rames,liste_lignes.length,50);
    ...
    function animation_graphique(Matrice,ctx) {
        ...
        class train {
            ...
            dessine_aller() {
                ...
                ctx.fillRect(this.position/10 - 10, 140, 20, 10);
            }
    
            dessine_retour() {
                ctx.fillStyle = '#FF0000';
                ctx.fillRect((33120-this.position)/10 - 10, 165, 20, 10);
            }
            ...
            avance(temps, Troncons) {
                ...
                if(this.aller && this.troncon == 73 && temps == this.horaire_suivant) {
                    this.aller = false;
                }
                ...
                if(temps == this.horaire_suivant && !this.marche) {
                    this.marche = true;
                    this.horaire_suivant = this.horaire_suivant + Math.trunc(Troncons[this.troncon] / Vmoy);
                }
                ...
                if(temps == this.horaire_suivant && this.marche) {
                    ...
                    // Pas d'arrêt
                    if(temps_d_arret == 0) {
                        this.horaire_suivant = this.tps_de_depart[this.troncon] + Math.trunc(Troncons[this.troncon] / Vmoy);
                    }
                    // Arrêt
                    else {
                        this.marche = false;
        ...
        function boucle(t) {
            dessine_Ligne_1();
            for(var i=0; i<Nombre_de_train; i++) {
                Tableau_train[i].avance(t[0], Troncons);
            }
            t[0] += 1;
        }
    
        setInterval(boucle, 5, temps);
    }    
